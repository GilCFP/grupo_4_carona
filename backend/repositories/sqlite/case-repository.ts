...
import { parseJson, safeStringify } from "../../utils/json.js";
import type {
...
function mapCaseEffectivenessRecord(effectiveness: {
  id: string;
  caseId: string;
  analysisId: string;
  finalOutcome: string;
  ...
  return {
    id: effectiveness.id,
    caseId: effectiveness.caseId,
    analysisId: effectiveness.analysisId,
    finalOutcome:
      effectiveness.finalOutcome === "agreement" ||
      effectiveness.finalOutcome === "judgment" ||
      effectiveness.finalOutcome === "extinct"
        ? effectiveness.finalOutcome
        : "extinct",
    actualAmountBrl: effectiveness.actualAmountBrl,
    createdAt: effectiveness.createdAt.toISOString(),
    updatedAt: effectiveness.updatedAt.toISOString()
  };
}
...
export async function getCaseEffectiveness(
  ...
  const effectiveness = await prisma.caseEffectiveness.findUnique({
    where: {
      caseId
    }
  });

  return effectiveness ? mapCaseEffectivenessRecord(effectiveness) : null;
}

export async function saveCaseEffectiveness(
  caseId: string,
  input: SaveCaseEffectivenessInput
): Promise<CaseEffectivenessRecord> {
  const existingEffectiveness = await prisma.caseEffectiveness.findUnique({
    where: {
      caseId
    }
  });
  ...
  if (existingEffectiveness) {
    throw new Error("Monitoramento de efetividade ja registrado.");
  }
  ...
  const actualAmountBrl =
    input.finalOutcome === "extinct" ? null : input.actualAmountBrl ?? null;

  const effectiveness = await prisma.caseEffectiveness.create({
    data: {
      caseId,
      analysisId: resolvedAnalysisId,
  ...
  return mapCaseEffectivenessRecord(effectiveness);
}
