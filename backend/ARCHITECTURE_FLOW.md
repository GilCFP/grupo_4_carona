...
A integração atual do `backend` está organizada a partir do composition root em `index.ts`, das rotas em `transportlayers/api/registerBackendApis.ts` e dos grafos em `graphs/policy-calibration-graph.ts` e `graphs/case-decision-graph.ts`.
