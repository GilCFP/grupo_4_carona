...
} from "../repositories/sqlite/case-repository.js";
import {
  getDashboardAdherence,
  getDashboardEffectiveness,
  getFeedbackSavingsSummary,
  getDashboardSummary
} from "../repositories/sqlite/dashboard-repository.js";
import {
  getActivePolicy,
  getPolicyByVersion,
  listPolicies
...
export class SQLiteDataSource implements SQLiteRepository {
  ...
  async getCaseEffectiveness(caseId: string) {
    return getCaseEffectiveness(caseId);
  }

  async saveCaseEffectiveness(caseId: string, input: Parameters<typeof saveCaseEffectiveness>[1]) {
    return saveCaseEffectiveness(caseId, input);
  }
