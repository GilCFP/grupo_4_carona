...
- Esta estrutura fica isolada em `backend/`.
- Nenhum arquivo do runtime atual foi redirecionado para ela.
- As implementacoes do `backend` agora foram internalizadas dentro desta propria arvore, sem depender de codigo legado externo.

## Comandos

- Subir o backend:
  - `npm run backend:api:start`
- Rodar demo do workflow1 + workflow2 no backend:
  - `npm run backend:demo:workflows`
- Validar tipagem e integracao do backend:
